...
describe('FileStateAdapter', () => {
  ...
  it('merges legacy vscode config section into settings on read', () => {
    const configDir = path.join(tempHome, '.pixel-agents');
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(
      path.join(configDir, 'config.json'),
      JSON.stringify({
        vscode: { watchAllSessions: true },
        standalone: { watchAllSessions: false },
      }),
      'utf-8',
    );

    const adapter = new FileStateAdapter();
    expect(adapter.getSetting('pixel-agents.watchAllSessions', null)).toBe(false);
  });
  ...
  it('persists settings under settings in config.json with clean field names', () => {
    const adapter = new FileStateAdapter();
    adapter.setSetting('pixel-agents.soundEnabled', false);
    const configPath = path.join(tempHome, '.pixel-agents', 'config.json');
  ...
  it('round-trips agents to standalone-state.json', () => {
    const adapter = new FileStateAdapter();
  ...
  it('writes state at ~/.pixel-agents/standalone-state.json', () => {
    const adapter = new FileStateAdapter();
